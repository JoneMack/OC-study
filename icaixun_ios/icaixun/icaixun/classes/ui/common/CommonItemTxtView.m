...
-(instancetype)initWithFrame:(CGRect)frame commonItemTxtArray:(NSArray *)commonItemTxtArray font:(UIFont *)font showMarkImg:(BOOL)showImg{
    self = [super init];
    if (self) {
        self.frame = frame;
        self.commonItemTxtArray = commonItemTxtArray;
        float padding = general_space/2;
        float y = padding;
        for (CommonItemTxt *itemTxt in commonItemTxtArray) {
            ...
            CGRect frame = CGRectMake(general_space, general_padding, general_space, general_space);
            self.markImgView = [[UIImageView alloc] initWithFrame:frame];
            self.markImgView.backgroundColor = [UIColor purpleColor];
            self.markImgView.image = [UIImage imageNamed:@"qbl"];
            [self addSubview:self.markImgView];

            if (showImg) {
               frame = CGRectMake(general_space+general_space+4, y-3, screen_width-general_margin*2, titleSize.height+padding);
            }else{
                frame = CGRectMake(general_space, y, screen_width-general_margin*2, titleSize.height+padding);
            }
            self.title = [[UILabel alloc] initWithFrame:frame];
            //对标题设置内容及样式
            self.title.backgroundColor = [UIColor clearColor];
            self.title.textColor = [ColorUtils colorWithHexString:green_common_color];
            self.title.font = [UIFont systemFontOfSize:bigger_font_size];
            ...
            [self.content setFrame:CGRectMake(general_space,y+self.title.frame.size.height, screen_width-30, size.height)];
