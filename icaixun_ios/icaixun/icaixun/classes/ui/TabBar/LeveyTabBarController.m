...
#define kTabBarHeight      tabbar_height
#define tabbarOffsetHeight 20
...
- (id)initWithViewControllers:(NSArray *)vcs imageArray:(NSArray *)arr andTitles:(NSArray *)titles
{
	...
	if (self != nil)
	{
		...
		_transitionView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, screen_width, _containerView.frame.size.height )];
        _transitionView.backgroundColor =  [UIColor groupTableViewBackgroundColor];
        [_transitionView setBackgroundColor:[UIColor clearColor]];

		
		_tabBar = [[LeveyTabBar alloc] initWithFrame:CGRectMake(0, _containerView.frame.size.height - kTabBarHeight, screen_width, kTabBarHeight) buttonImages:arr andItemTitle:titles];
