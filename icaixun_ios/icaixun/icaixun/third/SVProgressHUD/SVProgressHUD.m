...
+ (void)showWithStatus:(NSString*)status maskType:(SVProgressHUDMaskType)maskType duration:(NSTimeInterval)duration{
    ...
    [SVProgressHUD dismissWithSuccess:status afterDelay:duration];
}
...
+ (void)dismiss:(void(^)())completionBlock {
    [[SVProgressHUD sharedView] dismiss:completionBlock];
}
...
//        CGSize stringSize = [string sizeWithFont:self.stringLabel.font constrainedToSize:CGSizeMake(200, 300)];
        NSDictionary *attrDic = [NSDictionary dictionaryWithObjectsAndKeys:self.stringLabel.font, NSFontAttributeName, nil];
        CGSize stringSize = [string boundingRectWithSize:CGSizeMake(200, 300) options:NSStringDrawingUsesLineFragmentOrigin attributes:attrDic context:nil].size;
...
- (void)dismissWithStatus:(NSString *)string error:(BOOL)error afterDelay:(NSTimeInterval)seconds {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(error)
            self.imageView.image = [UIImage loadImageWithImageName:@"SVProgressHUD.bundle/error"];
        else
            self.imageView.image = [UIImage loadImageWithImageName:@"SVProgressHUD.bundle/success"];
...
- (UIWindow *)overlayWindow {
    if(!overlayWindow) {
        ...
        overlayWindow.windowLevel = UIWindowLevelStatusBar + 1.f;
