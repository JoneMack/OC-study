...
- (instancetype)initWithChatter:(NSString *)chatter conversationType:(EMConversationType)type
{
    ...
    if (self) {
        ...
        NSLog(@">>>>>>>>根据接收者的username获取当前会话的管理者>>>>>>>>>>>>_-------------------%@",_chatter);
...
- (void)setupBarButtonItem
{
    ...
    if (self.isChatGroup) {
        ...
        [detailButton setImage:[UIImage imageNamed:@"group_detail_icon"] forState:UIControlStateNormal];
...
{
    if (_chatToolBar == nil) {
        _chatToolBar = [[DXMessageToolBar alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - [DXMessageToolBar defaultHeight], kSCREEN_WIDTH, [DXMessageToolBar defaultHeight])];
...
//            NSLog(@">>>>>>>>>>>>>>>>>>>>>>>获取消息信息>>>>>>>>>>>>>>>>>>>>>%@",model);
            ...
            [MessageHandler getUserInfoByUserAccount:model.username success:^(BaseEntity *object) {
                ChatListUserEntity *chatEntity = (ChatListUserEntity *)object;
                NSDictionary *dict = chatEntity.dataDic;
                cell.nameLabel.text = dict[@"REAL_NAME"];
                [cell.headImageView sd_setImageWithURL:dict[@"thumb"] placeholderImage:[UIImage imageNamed:@"chatListCellHead.png"]];
            } fail:^(id object) {
...
{
    NSLog(@">>>>>>>>>>>>>>>>>>往datasource里添加的消息>>>>>>>>>>>>>>>>>>>%@",message);
    
    if ([_conversation.chatter isEqualToString:message.ext[@"real_receive_id"]]) {
        ...
        {
            BOOL result =[message updateMessageExtToDB];
    ...
    }else if ([_conversation.chatter isEqualToString:message.conversationChatter]) {
        [self addMessage:message];
        if ([self shouldAckMessage:message read:NO])
        {
            [self sendHasReadResponseForMessages:@[message]];
...
- (void)groupDidUpdateInfo:(EMGroup *)group error:(EMError *)error
{
    ...
    {
        NSString * nameString = group.groupSubject;
        NSRange range = [nameString rangeOfString:@"ZZDBDZKJYXGS"];
        if (range.length >0) {
            
            NSString * result = [nameString substringFromIndex:range.location];
            NSLog(@"%@",result);
            nameString = [[nameString componentsSeparatedByString:@"ZZDBDZKJYXGS"] safeObjectAtIndex:0];
        }
...
- (void)showRoomContact:(id)sender
{
    ...
    if (self.conversationType == eConversationTypeGroupChat) {
        ...
        detailController.friend_type = self.friend_type;
        detailController.qunGroup_type = self.qunGroup_type;
...
-(void)sendTextMessage:(NSString *)textMessage
{
    ...
    if ([Utils isBlankString: textMessage]) {
        [self showString: @"不能发送空白消息"];
        return;
    }
    
    if ([Utils isBlankString: _conversation.chatter]) {
        return;
    }
    EMMessage *tempMessage;
    if ([self.msg_type isEqualToString:doctor_to_patient]) {
       tempMessage  = [ChatSendHelper sendTextMessageWithString:textMessage
                                                                toUsername:_conversation.chatter
                                                               messageType:[self messageType]
                                                         requireEncryption:NO
                                                                       ext:ext];
        [ext setValue:[GlobalConst shareInstance].loginInfo.login_name forKey:@"real_receive_id"];
        [ext setValue:@"app" forKey:@"msg_type"];
        tempMessage = [ChatSendHelper sendTextMessageWithString:textMessage
                                                     toUsername:[GlobalConst shareInstance].loginInfo.login_name
                                                    messageType:[self messageType]
                                              requireEncryption:NO
                                                            ext:ext];
    }else{
    ...
    NSLog(@">>>>>>>>>>>>tempMessage.ext>>>tempMessage>>>>>>%@>>>>>%@",tempMessage,tempMessage.ext);
    if ([self.chatToType isEqualToString:patient_to_doctor]) {
       [self serviceForAssistantWithMsgContent:tempMessage.from acceptId:tempMessage.to msgContent:@"发送的是一条语音"];    }
...
-(void)sendImageMessage:(UIImage *)image
{
    ...
    if ([self.chatToType isEqualToString:patient_to_doctor]) {
        ...
        [self serviceForAssistantWithMsgContent:tempMessage.from acceptId:tempMessage.to msgContent:txt];
    }
...
-(void)sendVideoMessage:(EMChatVideo *)video
{
    ...
    EMMessage *tempMessage = [ChatSendHelper sendVideo:video
                                     ...
                                     requireEncryption:NO ext:ext];
    if ([self.chatToType isEqualToString:patient_to_doctor]) {
        [self serviceForAssistantWithMsgContent:tempMessage.from acceptId:tempMessage.to msgContent:@"发送的是一条视频"];
    }
...
//                NSString *txt = ((EMTextMessageBody *)msgBody).text;
//                [self serviceForAssistantWithMsgContent:message.from acceptId:message.to msgContent:txt];
//            }
...
- (void)serviceForAssistantWithMsgContent:(NSString *)sendId  acceptId:(NSString *)acceptId  msgContent:(NSString *)msgContent{
    NSMutableDictionary *dic = [NSMutableDictionary dictionary];
    [dic safeSetObject:@"" forKey:@"accountId"];
    [dic safeSetObject:sendId forKey:@"sendId"];
    [dic safeSetObject:acceptId forKey:@"acceptId"];
    [dic safeSetObject:@"1" forKey:@"msgType"];
