...
@property (strong, nonatomic) NSMutableArray *frienUserInfo;
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        _frienUserInfo = [NSMutableArray array];
...
//    [self.tableView reloadData];
    ...
    [MyPatientHandler getAllGroupWithType:self.friend_IDType success:^(BaseEntity *object) {  // 此处传来的type不准确，1 医生   ；    0 患者
        
        if (object.success) {
            
            self.frienUserInfo = [NSMutableArray new];
            GroupListEntity *entity = (GroupListEntity *)object;
            
            NSLog(@"entity.dataentity.dataentity.dataentity.dataentity.data--->%@",entity.data);
            for (GroupInfoEntity *friendGroup in entity.data)
            {
                NSLog(@"-friendGroup-->%@",friendGroup);
                [self.frienUserInfo addObjectsFromArray:friendGroup.friends];
            }
            
            NSLog(@"--->%@",self.frienUserInfo);
            
            NSArray *buddyList = [[EaseMob sharedInstance].chatManager buddyList];
            for (EMBuddy *buddy in buddyList) {
                if (buddy.followState != eEMBuddyFollowState_NotFollowed) {
                    
                    for (UserEntity *friend in self.frienUserInfo) {
                        
                        if ([buddy.username isEqualToString:friend.LOGIN_NAME]) {
                            [self.contactsSource addObject:buddy];
                        }
