...
@property (strong, nonatomic) NSString *groupDiscribeString;

@property (strong, nonatomic) NSMutableArray *patUserInfoArray;
@property (strong, nonatomic) NSArray *getPhoneNum;
...
- (instancetype)initWithGroupId:(NSString *)chatGroupId
{
    ...
    for (EMGroup *group in groupArray) {
        NSLog(@">>>>>>>>>>群组成员>>>>>>>>>>%@",group.members);
        if ([group.groupId isEqualToString:chatGroupId]) {
            chatGroup = group;
            self.groupDiscribeString = group.groupDescription;
            break;
        }
    }
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    self.getPhoneNum = [[NSArray alloc]init];
    ...
    [self fetchGroupInfo];
    [self startGetGroupListForHuanZhe];
}
...
- (void)startGetGroupListForHuanZhe
{
    NSRange range = [self.groupDiscribeString rangeOfString:@"卍_"];
    if (range.length >0) {
        
        self.getPhoneNum = [self.groupDiscribeString componentsSeparatedByString:@"卍_"];
    }
    
    NSLog(@"%@",[self.getPhoneNum safeObjectAtIndex:1]);
    if (self.getPhoneNum.count == 0) {
        return;
    }
    NSString *account = [self.getPhoneNum safeObjectAtIndex:1];
    NSLog(@"%@",account);
    self.patUserInfoArray = [[NSMutableArray alloc]init];
    ...
    [[GRNetworkAgent sharedInstance] requestUrl:getUserInfo_by_username_url param:dic baseUrl:BASEURL withRequestMethod:GRRequestMethodGet withCompletionBlockWithSuccess:^(GRBaseRequest *request, id responeObect) {
        ...
        NSDictionary *dic = [responeObect safeObjectForKey:@"data"];
        
        UserEntity *friend = [UserEntity objectWithKeyValues:dic];
        [self.patUserInfoArray removeAllObjects];
        [self.patUserInfoArray safeAddObject:friend];
        
        NSLog(@"%@",friend.REAL_NAME);
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    {
        if ([self.qunGroup_type isEqualToString:huizhen_group_list]) {
            return 5;
        }
        return 4;
    }
...
//    }
    ...
    {
        cell.textLabel.text = @"病情详情";//self.groupDiscribeString;
        cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if (indexPath.row == 2)
    {
        ...
        changingController.qunGroup_Type = self.qunGroup_type;
        [self.navigationController pushViewController:changingController animated:YES];
    }
    else if (indexPath.row == 3) {
        if ([self.qunGroup_type isEqualToString:huizhen_group_list])
        {
            if (self.patUserInfoArray.count == 0) {
                [self showHint:@"无患者信息"];
                return;
            }
            NSInteger countObj = self.patUserInfoArray.count;
            UserEntity *user = [self.patUserInfoArray safeObjectAtIndex:countObj-1];
            PatientDetailController *patDe = [PatientDetailController simpleInstance];
            patDe.userInfoEntity = user;
            [self.navigationController pushViewController:patDe animated:YES];
        }
        else
        {
    ...
    else if (indexPath.row == 4)
    {
        ...
        NSString *discribeStr = [self.getPhoneNum safeObjectAtIndex:0];
        if (discribeStr.length >0)
        {
            GroupDiscribeViewController *discib = [[GroupDiscribeViewController alloc]init];
            discib.groupDiscribe = discribeStr;
            [self.navigationController pushViewController:discib animated:YES];
        }
        else
        {
            [self showHint:@"无详细信息"];
...
- (void)addContact:(id)sender
{
    ...
    selectionController.friend_IDType = self.friend_type;
