...
- (void)viewDidLoad
{
    ...
    /*[self.view addSubview:self.switchView];*//*此处是switch的方法，由于改变界面不需展示，注释于此*/
}
...
{
    if (_textField == nil) {
        _textField = [[UITextField alloc] initWithFrame:CGRectMake(10, 10, kSCREEN_WIDTH-20, 40)];
...
{
    if (_textView == nil) {
        _textView = [[EMTextView alloc] initWithFrame:CGRectMake(10, 70, kSCREEN_WIDTH-20, 80)];
...
/*此处是switchView的方法，由于改变界面不需展示，注释于此*/
/*- (UIView *)switchView
{
    if (_switchView == nil) {
...
//    }
    ...
    NSString *tmpGroupName;
    if ([self.navTitle isEqualToString:@"患者群组"]){
        tmpGroupName = [NSString stringWithFormat:@"%@ZZDBDZKJYXGS0",self.textField.text];
    }
    else if ([self.navTitle isEqualToString:@"同行群组"]) {
        tmpGroupName = [NSString stringWithFormat:@"%@ZZDBDZKJYXGS1",self.textField.text];
    }else{
        tmpGroupName = [NSString stringWithFormat:@"%@ZZDBDZKJYXGS2",self.textField.text];
    }
    
    NSString *messageStr = [NSString stringWithFormat:NSLocalizedString(@"group.somebodyInvite", @"%@ invite you to join groups \'%@\'"), username, tmpGroupName];
    [[EaseMob sharedInstance].chatManager asyncCreateGroupWithSubject:tmpGroupName description:self.textView.text invitees:source initialWelcomeMessage:messageStr styleSetting:setting completion:^(EMGroup *group, EMError *error) {
        [weakSelf hideHud];
        if (group && !error) {
...
- (void)addContacts:(id)sender
{
    ...
    selectionController.friend_IDType = self.friend_IDType;
