...
//    [self.navigationItem setLeftBarButtonItem:backItem];
    ...
    NSString *placeHoldText ;
    placeHoldText = _group.groupSubject;
    NSRange range = [placeHoldText rangeOfString:@"ZZDBDZKJYXGS"];
    if (range.length >0) {
        
        NSString * result = [placeHoldText substringFromIndex:range.location];
        NSLog(@"%@",result);
        placeHoldText = [[placeHoldText componentsSeparatedByString:@"ZZDBDZKJYXGS"] safeObjectAtIndex:0];
    }
    _subjectField.text = placeHoldText;
...
- (void)saveSubject
{
    ...
    if ([self.qunGroup_Type  isEqual: huizhen_group_list])
    {
        resetName = [NSString stringWithFormat:@"%@ZZDBDZKJYXGS2" ,_subjectField.text];
    }
    if ([self.qunGroup_Type isEqualToString:tongHang_group_list])
    {
        resetName = [NSString stringWithFormat:@"%@ZZDBDZKJYXGS1" ,_subjectField.text];
    }
    if ([self.qunGroup_Type isEqualToString:patients_group_list])
    {
        resetName = [NSString stringWithFormat:@"%@ZZDBDZKJYXGS0" ,_subjectField.text];
    }
    
    [[EaseMob sharedInstance].chatManager asyncChangeGroupSubject:resetName forGroup:_group.groupId];
}
