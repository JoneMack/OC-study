...
#warning SDK注册 APNS文件的名字, 需要与后台上传证书时的名字一一对应
    ...
    if (![self isSpecifyServer]) {
        [[EaseMob sharedInstance] registerSDKWithAppKey:easemob_app_key
...
-(void)willAutoLoginWithInfo:(NSDictionary *)loginInfo error:(EMError *)error
{
    ...
    if (error) {
        ...
        alertView = [[UIAlertView alloc] initWithTitle:@"自动登录..." message:@"" delegate:nil cancelButtonTitle:@"ok" otherButtonTitles:nil, nil];
...
-(void)didAutoLoginWithInfo:(NSDictionary *)loginInfo error:(EMError *)error
{
    ...
    if (error) {
       ...
       // [[NSNotificationCenter defaultCenter] postNotificationName:KNOTIFICATION_LOGINCHANGE object:@NO];
        ...
        alertView = [[UIAlertView alloc] initWithTitle:@"" message:@"" delegate:nil cancelButtonTitle:@"" otherButtonTitles:nil, nil];
    }
                               ...
                               error:(EMError *)error
{
    ...
    NSRange range = [groupTag rangeOfString:@"ZZDBDZKJYXGS"];
    if (range.length >0) {
        NSString * result = [groupTag substringFromIndex:range.location];
        NSLog(@"%@",result);
        groupTag = [[groupTag componentsSeparatedByString:@"ZZDBDZKJYXGS"] safeObjectAtIndex:0];
    }
    ...
    if([message rangeOfString:@"-的会诊'"].location !=NSNotFound){
        //发送本地推送
        UILocalNotification *notification = [[UILocalNotification alloc] init];
        notification.fireDate = [NSDate date]; //触发通知的时间
        notification.alertBody=@"接到新的会诊";
