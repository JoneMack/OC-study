...
@implementation UpdatePassLockCell
...
-(void) renderView{
    ...
    self.tixingLabel.frame = CGRectMake(0, 22, screen_width, 13);
    [self.tixingLabel setText:@"请输入正在使用的解锁密码"];
    ...
    self.bottomBtn.frame = CGRectMake(30, self.btn0.bottomY+45, screen_width-60, 49);
    ...
    [self.bottomBtn setBackgroundColor:[ColorUtils colorWithHexString:bg_purple alpha:0.5]];
    [self.contentView addSubview:self.bottomBtn];
    
    [self.bottomBtn addTarget:self action:@selector(nextBtnEvent) forControlEvents:UIControlEventTouchUpInside];
    
    self.currentStatus = @"old";
...
-(void) nextBtnEvent
{
    if([self.currentStatus isEqualToString:@"old"]){
        [self.tixingLabel setText:@"请输入您要修改的解锁密码"];
        self.currentStatus = @"new";
    }else if([self.currentStatus isEqualToString:@"new"]){
        [self.tixingLabel setText:@"请再次输入您要修改的解锁密码"];
        [self.bottomBtn setTitle:@"完成修改" forState:UIControlStateNormal];
        self.currentStatus = @"confirm";
    }else if([self.currentStatus isEqualToString:@"confirm"]){
