//
//  LookingUnFinishViewController.m
//  xiangyu
//
...
#import "HouseStore.h"
@interface LookingUnFinishViewController ()
...
@implementation LookingUnFinishViewController
...
- (void)viewDidLoad {
    ...
    [self.selectBtn addTarget:self action:@selector(selectedUnFnishBtn:) forControlEvents:UIControlEventTouchUpInside];
    self.selectedUnfnishHouseIdArray = [NSMutableArray new];
    [self initTableView];
...
- (void)loadData{
    ...
    [[HouseStore sharedStore] getUserUnfinishedOrderList:^(NSArray *userOrderDetails, NSError *err) {
        [self.tableView.mj_header endRefreshing];
        [self.tableView.mj_footer endRefreshing];
        self.currentTableViewStatus = load_data_status_waiting_load;
        if(self.lookingUnfnishHouseArray == nil){
            self.lookingUnfnishHouseArray = [NSMutableArray new];
        }
        if(userOrderDetails != nil && userOrderDetails.count >0){
            [self.lookingUnfnishHouseArray addObjectsFromArray:userOrderDetails];
            
        }else{
            [self.tableView.mj_footer endRefreshingWithNoMoreData];
            self.currentTableViewStatus = load_data_status_load_over;
        }
        NSLog(@">>>>>>未完成约看>>>>>>%@------%@",userOrderDetails,self.lookingUnfnishHouseArray);
        [self initModelData];
...
- (void)initModelData{
    if (self.lookingUnfnishHouseArray.count > 0) {
        [self.lookingUnfnishHouseArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            if (_cells == nil) {
...
{
    self.currentTableViewStatus = load_data_status_waiting_load;
    [self.lookingUnfnishHouseArray removeAllObjects];
...
//选择按钮点击响应事件
- (void)selectedUnFnishBtn:(UIButton *)sender{
    //支持同时选中多行
...
- (void)checkCell:(UITapGestureRecognizer *)tapGesture {
    ...
    if (obj.isSelected == YES) {
        [self.selectedUnfnishHouseIdArray addObject:@(obj.orderId)];
    }else{
        [self.selectedUnfnishHouseIdArray removeObject:@(obj.orderId)];
    }
    NSLog(@">>>>>>selectedUnfnishHouseIdArray>>>>>>>%@",self.selectedUnfnishHouseIdArray);
    [self.tableView reloadData];
}
...
- (void)rightDeleteBtnClick{
    ...
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        } ids:[self.selectedUnfnishHouseIdArray componentsJoinedByString:@","]];
    }];
    //实例化确定按钮
    //向弹出框中添加按钮和文本框
    [alertController addAction:cancelAction];
...
-(void) transformEvent:(int)eventType{
    ...
    if ((self.currentTableViewStatus == load_data_status_waiting_load
        ...
        // 初始化加载
        if(self.lookingUnfnishHouseArray != nil && self.lookingUnfnishHouseArray.count >0){
            [self.lookingUnfnishHouseArray removeAllObjects];
        }
