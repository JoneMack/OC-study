...
{
    NSArray *btnTitleArray = [[NSArray alloc] initWithObjects:@"未使用",@"未付款",@"已使用",@"退款",@"赠送", nil];
...
-(int)getSelectIndex{
    if ([self.currentCardStatuses isEqualToArray:[[NSArray alloc] initWithObjects:@(user_card_status_paid), nil]]) {
        ...
        return 3;
    }else if ([self.currentCardStatuses isEqualToArray:[[NSArray alloc] initWithObjects:@(user_card_status_donate) , nil]]){
        return 4;
    }
    return 0;
}
...
-(void)selectSegment:(int)inx
{
    switch (inx) {
        ...
        case 3:
            ...
            break;
        case 4:
            self.currentCardStatuses = [[NSArray alloc] initWithObjects:@(user_card_status_donate), nil];
            [MobClick event:log_event_name_view_my_hdcs attributes:[NSDictionary dictionaryWithObjectsAndKeys:
                                                                    @"赠送", @"类型", nil]];
        default:
            break;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == ([self.userHdcsTableView numberOfRowsInSection:0]-1)) {
        ...
        if ([self.currentCardStatuses isEqualToArray:[[NSArray alloc] initWithObjects:@(user_card_status_donate) , nil]]) {
            cell.lockImgView.hidden = NO;
            float alpha = 0.6;
            cell.redEnvelopeLabel.alpha = alpha;
            cell.lockImgView.alpha = 1;
            cell.hdcIconImgView.alpha = alpha;
            cell.priceLabel.alpha = alpha;
            cell.hdcTitleLabel.alpha = alpha;
            cell.organizationNameLabel.alpha = alpha;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == ([tableView numberOfRowsInSection:0]-1)) {
        ...
        if (userHdc.orderItemStatus == user_card_status_unpayment)
        {
            ...
            if (userHdc.hdcReward != nil ) {
                // 如果是赠送的美发卡列表就什么也不做
                return ;
            }else if (userHdc.orderItemStatus == user_card_status_paid){
