...
#define lock_img_width       18
#define lock_img_height      24
...
-(void)render:(UserHdc *)card organization:(Organization *)organization withSplite:(BOOL)withSplite last:(BOOL)last{
    ...
    if (card.orderItemStatus == user_card_status_unpayment || [card isRewardedUserHdc] ) {
    ...
    if (self.lockImgView == nil && self.userHdc.hdcReward != nil) {
        self.lockImgView = [[UIImageView alloc] initWithFrame:CGRectMake(general_padding+(60-lock_img_width)/2, general_padding+(60-lock_img_height)/2, lock_img_width, lock_img_height)];
        self.lockImgView.image = [UIImage imageNamed:@"icon_locked"];
...
-(UIColor *) getStatusBgColor:(UserHdc *)card{
    UIColor *result = nil;
    if ([self.userHdc isRewardedUserHdc]) {
        return [ColorUtils colorWithHexString:blue_color];
    }else if ([self.userHdc isReceivedUserHdc]){
        return [ColorUtils colorWithHexString:orange_order_status_backgroud_color];
    }else if (self.userHdc.expiredStatus) {
        return [ColorUtils colorWithHexString:lighter_gray_text_color];
    }
    switch (card.orderItemStatus) {
        ...
        case 1:
            ...
            break;
        case user_card_status_paid: // 未使用
            result = [ColorUtils colorWithHexString:green_background_color];
            break;
        case user_card_status_request_refund: // 申请退款中
        case user_card_status_refunded: // 已退款
