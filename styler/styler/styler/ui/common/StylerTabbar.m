...
-(void)tabBarController:(LeveyTabBarController *)tabBarController didSelectViewController:(UIViewController *)viewController
{
    ...
    if([uc isKindOfClass:[ContentNavController class]]){
    ...
    if([uc isKindOfClass:[WorkListController class]]){
        WorkListController *workListController = (WorkListController *)uc;
        [workListController.workList removeAllObjects];
        [workListController.tableView reloadData];

        double delayInSeconds = 0.1;
        dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, delayInSeconds * NSEC_PER_SEC);
        dispatch_after(popTime, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
            [workListController refreshTableView];
        });
    }
}
