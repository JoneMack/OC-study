...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(showShareApppRewardActivity) name:notification_name_has_share_app_reward_activity object:nil];
    
    
    
    
    
    NewContact *newContact = [[NewContact alloc] init];
    newContact.userId = 16;
    newContact.name = @"dcb";
    newContact.phone = @"18811753356";
    newContact.userGender = 1;
    newContact.contactsType = 1;
    
    NSLog(@">>>>>>>contacts>>>>>>>>%@",newContact);
    
//    [SVProgressHUD showWithStatus:@"数据处理，请稍等..."];
    [[OrderStore sharedStore] addNewPlayer:^(Contact *contact,NSError *err) {
//        [SVProgressHUD dismiss];
        if (err == nil) {
            NSLog(@"___________添加成功___________");
            
        }else if(err != nil){
            NSLog(@"___________添加失败___________");
        }
    } contactsTo:newContact];
...
//    [self.header addSubview:searchBtn];
    ...
    float brandX = screen_width- 50;
    UIButton *brandBtn = [[UIButton alloc] initWithFrame:CGRectMake(brandX , self.header.backBut.frame.origin.y, navigation_height, navigation_height)];
    [brandBtn setTitle:@"品牌" forState:UIControlStateNormal];
    [brandBtn setTitleColor:[ColorUtils colorWithHexString:black_text_color] forState:UIControlStateNormal];
    ...
    [self.header addSubview:brandBtn];
    
}

-(void)gotoAllBrand{
    
    [self.navigationController pushViewController:[[AllBrandController alloc] init] animated:YES];
}
...
-(void) loadWebPage{
    ...
    NSString *bannerUrl = @"";
    if ([rap hasRewardActivityInSharedContentType:shareAppPage]) {
        bannerUrl = [[rap getActivityBannerUrl:shareOrganizationPage] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    };
    
    NSString * urlString = [NSString  stringWithFormat:@"%@/app/index?poi=%f,%f&bannerUrl=%@", as.webPageUrl, as.lastLat, as.lastLng ,bannerUrl];
    if (as.lastLat == 0 || as.lastLng == 0) {
        urlString = [NSString  stringWithFormat:@"%@/app/index?poi=%f,%f&bannerUrl=%@", as.webPageUrl, 39.974101, 116.348413 , bannerUrl];
    }
...
-(void) webViewDidFinishLoad:(UIWebView *)webView{
    ...
    [self showShareApppRewardActivity];
}
...
-(void) showShareApppRewardActivity{
    RewardActivityProcessor *rap = [RewardActivityProcessor sharedInstance];
    NSString *bannerUrl = @"";
    if ([rap hasRewardActivityInSharedContentType:shareAppPage]) {
        bannerUrl = [rap getActivityBannerUrl:shareAppPage];
    };
    
    NSString *rewardActivityBannerImgJS = [NSString stringWithFormat:
                                           @"var rewardActivityBannerImgElement = document.getElementById('rewardActivityBannerImg')"
                                           "rewardActivityBannerImgElement.src=%@" , bannerUrl] ;
    [self.webView stringByEvaluatingJavaScriptFromString:rewardActivityBannerImgJS];
