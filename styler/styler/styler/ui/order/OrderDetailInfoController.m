...
-(void) initStylistAndServiceItemView
{
    ...
    if (self.order.stylist.dataStatus != stylist_data_status_open) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 1){
        UITapGestureRecognizer *tapGestureTel = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(showOrganizationProfile)];
        [cell addGestureRecognizer:tapGestureTel];
    }
 ...
 *  查看商户详情页
 */
-(void) showOrganizationProfile{
    [OrganizationStore getOrganizationById:^(Organization *orginzation, NSError *err) {
        if (orginzation == nil || orginzation == NULL) {
            [SVProgressHUD showErrorWithStatus:@"该商户已下线" duration:1];
        }else{
            OrganizationDetailInfoController *organizationDetailInfoController = [[OrganizationDetailInfoController alloc] initWithOrganizationId:self.order.organizationId];
            [self.navigationController pushViewController:organizationDetailInfoController animated:YES];
        }
    } organizationId:self.order.organizationId hasStylists:YES];
}
