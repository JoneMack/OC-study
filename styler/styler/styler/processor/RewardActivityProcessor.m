...
typedef enum {
    reward_activity_hide_with_reward_count = 0,  // 用户领取奖励的次数达到上限时 隐藏奖励活动
    reward_activity_hide_with_reward_over  = 1,  // 奖项活动不存在时，隐藏奖励活动
}
RewardActivityControlStrategy;

#define current_reward_activity_show_control_strategy   reward_activity_hide_with_reward_over
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        ...
        if (actionTypes == nil) {
            actionTypes = [NSArray arrayWithObjects:[NSString stringWithFormat:@"%d", shareOrganizationPage],
                           ...
                           [NSString stringWithFormat:@"%d", shareAppPage],
                           nil];
        }
            ...
            [RewardActivityStore getRewardActivity:^(RewardActivity *rewardActivity, NSError *error) {
                if (rewardActivity != nil && rewardActivity.id > 0) {
                    ...
                    [RedEnvelopeStore getRedEnvelopeSeedById:^(RedEnvelopeSeed *redEnvelopeSeed, NSError *error) {
                        ...
                        if ([actionType intValue] == shareAppPage) {
                            [[NSNotificationCenter defaultCenter] postNotificationName:notification_name_has_share_app_reward_activity object:nil];
                        }
//                        NSLog(@">>>>>类型%@有奖励活动.", actionType);
                    ...
                    if ([actionType isEqualToString:[NSString stringWithFormat:@"%d" , shareAppPage]]) {
                        [[NSNotificationCenter defaultCenter] postNotificationName:notification_name_has_share_app_reward_activity object:nil];
                    }
                }
...
-(void) tryDisplayMovement:(UIViewController *)vc{
    ...
    }else if([vc isKindOfClass:[WorkListController class]]){
        remindContent = @"时尚猫";
        currentActionTypeShared = shareAppPage;
        WorkListController *workListController = (WorkListController *)vc;
        shareContent = [workListController collectionShareContent];
    }else if([vc isKindOfClass:[ContentNavController class]]){
        remindContent = @"时尚猫";
        currentActionTypeShared = shareAppPage;
        ContentNavController *contentNavController = (ContentNavController *)vc;
        shareContent = [contentNavController collectionShareContent];
    }else if([vc isKindOfClass:[ShareEnableWebController class]]){
        remindContent = @"此美发卡";
        currentActionTypeShared = shareHdcPage;
        ShareEnableWebController *shareEnableWebController = (ShareEnableWebController *)vc;
        shareContent = [shareEnableWebController collectionShareContent];
    }
    ...
    if ([NSStringUtils isNotBlank:remindContent] && currentActionTypeShared>0) {
        ...
        NSString *activityDescription = [NSString stringWithFormat:@"推荐%@到微信朋友圈得红包，最多%d元！",
...
-(void)showInputMobileNoAndGetReward:(UserAction *)userAction{
    [SVProgressHUD dismiss:^{
        ...
        NSString *title = [NSString stringWithFormat:@" 推荐%@到微信朋友圈得红包，最多%d元！",remindContent,maxAmount];
    ...
    dispatch_after(popTime, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        [RewardActivityStore getUserAction:^(UserAction *userAction, NSError *error) {
            if (userAction.rewardStatus == action_activity_result_no_reward ) {
                ...
                [self displayhasRewardResultView:[NSString stringWithFormat:@"您已经领了%d个红包，我们还为您准备了%d个红包，赶紧分享给小伙伴们！" , as.rewardActivityEachUserGetRedEnvelopeCount, as.rewardActivityUserShareRedEnvelopeCount]];
            }else{
                [self getUserActionReward:userAction];
            }
...
-(void) displayNoRewardResultView{
    [SVProgressHUD dismiss:^{
        ...
        if (current_reward_activity_show_control_strategy != reward_activity_hide_with_reward_over) {
