...
+(BOOL) dispatch:(NSURL *)url nav:(UINavigationController *)nav
{
    ...
    if ([host isEqualToString:@"styler.meilizhuanjia.cn"] || [host isEqualToString:@"styler.shishangmao.cn"]) {
        ...
        if([path rangeOfString:@"/index"].location < path.length)
        {
            ...
            if ([path rangeOfString:@"/rewardActivity"].location < path.length) {  // 点击了主页的banner分享
                UIViewController *currentContentNavController = [nav viewControllers][nav.viewControllers.count -1];
                RewardActivityProcessor *rewardActivityProcessor = [RewardActivityProcessor sharedInstance];
                [rewardActivityProcessor tryDisplayMovement:currentContentNavController];
                [MobClick event:log_event_name_share_app_reward_activity_by_index];
                
                return YES;
            }
            
            if ([path rangeOfString:@"/businessCircles"].location < path.length) {  // 点击了全部商圈
                if ([path rangeOfString:@"/all"].location < path.length) {
                    [nav pushViewController:[[AllBusinessCirclesController alloc] init] animated:YES];
                    return YES;
                }else{
//                  path demo: /index/businessCircles?citydistrictName=%s&businessCircleName=%s&businessCircleId=%d
                    params = [params stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
                    NSMutableDictionary *paramsDict = [params urlParamsChange2Dict];
                    if (paramsDict.count == 3) {
                        OrganizationFilter *organizationFilter = [[OrganizationFilter alloc] initWithSelectedBusinessCircle:[paramsDict valueForKey:@"citydistrictName"] selectedBusinessCircleId:[[paramsDict valueForKey:@"businessCircleId"] intValue] selectedBusinessCircleName:[paramsDict valueForKey:@"businessCircleName"]];
                        ...
                        [nav pushViewController:organizationSpecialOfferListViewController animated:YES];
                        [MobClick event:log_event_name_select_businesscirlces attributes:[NSDictionary dictionaryWithObjectsAndKeys:[paramsDict valueForKey:@"businessCircleName"], @"商圈", nil]];
                    }
                    return YES;
                }
            }
...
//        }
            ...
            if([path rangeOfString:@"rewardActivity"].location < path.length){  // 点击了商户详情页的奖励活动banner
            ...
            if([path hasPrefix:@"/preferential"]){
                ...
                if ([paramKeys containsObject:@"hdcTypeId"]) {
                    params = [params stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
                    NSMutableDictionary *dics = [params urlParamsChange2Dict];
                    organizationFilter.selectedHdcTypeId = [[dics valueForKey:@"hdcTypeId"] intValue];
                    organizationFilter.selectedHdcTypeName = [dics valueForKey:@"hdcTypeName"];
                    organizationFilter.selectedHdcCatalogName = [dics valueForKey:@"hdcCatalogName"];
                    ...
                    [MobClick event:log_event_name_hdc_type
                         attributes:[NSDictionary dictionaryWithObjectsAndKeys:@(organizationFilter.selectedHdcTypeId),
                                                                               @"美发卡类型", nil]];
        ...
        else if([path rangeOfString:@"/hairDressingCards/"].location < path.length){  // 特惠精选进入美发卡详情页
            
            if([path rangeOfString:@"rewardActivity"].location < path.length){  // 点击了美发卡详情页的奖励活动banner
                
                NSString *str = @"/hairDressingCards/";
                int hdcId = [[path substringFromIndex:str.length] intValue];
                
                UIViewController *currentHdcDetailController = [nav viewControllers][nav.viewControllers.count -1];
                RewardActivityProcessor *rewardActivityProcessor = [RewardActivityProcessor sharedInstance];
                [rewardActivityProcessor tryDisplayMovement:currentHdcDetailController];
                
                [MobClick event:log_event_name_share_hdc_reward_activity attributes:[NSDictionary dictionaryWithObjectsAndKeys:@(hdcId), @"美发卡id" , nil]];
                return YES;
            }
...
+(BOOL) dispatchWithContentSort:(int)contentSortId
    ...
    switch (contentModeType) {
        case 1://标签作品；
            ...
            wclc = [[WorkListController alloc] initWithRequestURL:url title:contentSortName type:tag_name_work_by_content_sort];
