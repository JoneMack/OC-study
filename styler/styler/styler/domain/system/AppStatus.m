...
#import <ThirdFramework/OpenUDID/OpenUDID.h>
...
-(BOOL) isDefaultUserName{
    if (self.user == nil) {
        return NO;
    }
    if ([self.user.name isEqualToString:@"用户"]) {
        return YES;
    }
    return NO;
}
...
+(AppStatus *)sharedInstance{
    ...
    if(sharedInstance == nil){
        ...
        sharedInstance.hasNewStylistWorks = NO;
    }
 ...
 */
-(void) initBaseData{
    self.feedbackSessionId = -1;
    self.hasUnevaluationOrder = NO;
    self.hasUnpaymentHdc = NO;
    self.evaluatedOrderIds = [NSMutableArray new];
    self.appBecomeActiveScene = fromAppLaunch;
    self.user = nil;
}
