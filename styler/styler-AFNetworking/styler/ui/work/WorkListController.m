...
-(void) handleLoadData:(Page *)page{
    if (page != nil) {
        ...
        [workList addObjectsFromArray:page.items];
...
{
    return workList.count+2;
}

-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == (workList.count+1) || indexPath.row == 0) {
        return loading_view_height;
    }
    
    StylistWork *work = workList[indexPath.row-1];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell initUI:workList[indexPath.row-1] viewController:self];
...
-(void)deleteWorkBtnClick:(UIButton *)sender{
    StylistWork *work = [workList objectAtIndex:currentRow-1];
    ...
    [[UserStore sharedStore] removeFavWork:^(NSError *err) {
        if (err == nil) {
            ...
            [workList removeObjectAtIndex:currentRow-1];
...
{
    if (indexPath.row == workList.count) {
        return NO;
    }
    return YES;
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row != workList.count+1) {
        StylistWork *work = [workList objectAtIndex:indexPath.row-1];
        ...
        wdc.work = [workList objectAtIndex:indexPath.row-1];
        [self.navigationController pushViewController:wdc animated:YES];
    }else if(indexPath.row == workList.count){
        [self loadWorkList];
    }
}
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    //计算当前位置
    if (((int)currentOffsetY) > loading_view_height && (currentOffsetY < (self.tableView.contentSize.height - self.tableView.frame.size.height))) {
    ...
    }else if(currentOffsetY >= (self.tableView.contentSize.height - self.tableView.frame.size.height)){
