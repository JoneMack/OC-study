...
-(void)loadHdcs
{
    ...
    [HdcStore searchHdc:^(Page *page, NSError *error) {
        if (error == nil && page.items.count>0) {
           self.hdcs = [HairDressingCard sortHdcs:page.items];
        }
        [self.tableView reloadData];
    } query:query];
}
...
-(CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section
{
    if (section == 0) {
        ...
        }else if ([self hasRewardActivity]) {
            return 0;
        }
        else{
    ...
    downSpliteLine.backgroundColor = [ColorUtils colorWithHexString:splite_line_color];
        if (indexPath.section == 0) {//发型师信息，简介
            if (indexPath.row == 0) {
                ...
                return cell;
            }else if (indexPath.row == 1){//个人简介和个人价目表
...
//                [cell addSubview:arrow];
            ...
            if ([self hasRewardActivity]) {
                ...
                rewardActivityBtn.frame = CGRectMake(0, 0, screen_width, general_cell_height);
        ...
        else if (indexPath.section == 3)//在售美发卡
        {
            ...
            for (HairDressingCard *hdc in self.hdcs[indexPath.row]) {
                CGRect frame = CGRectMake(general_padding, y, self.view.frame.size.width-general_padding, cell.frame.size.height);
                OrgHdcView *hdcView = [[OrgHdcView alloc] initWithFrame:frame hdc:hdc displayScene:display_in_stylist_profile];
...
#pragma mark - RewardActivityProcessorDelegate
