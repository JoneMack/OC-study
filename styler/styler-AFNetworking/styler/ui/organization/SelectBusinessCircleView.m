...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    // 处理最后一个分隔线
    if ([[[currentBusinessCircles lastObject] name] isEqualToString:cell.textLabel.text]) {
...
-(id)initWithBusinessCircleName:(NSString *)businessCircleName reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        self.separatorLine.frame = CGRectMake( business_circle_content_margin_left, self.frame.size.height-2 , self.frame.size.width-business_circle_content_margin_left,  splite_line_height);
