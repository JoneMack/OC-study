...
@property (nonatomic, retain) AppStatus *as;
...
- (id)initWithChatter:(NSString *)chatter
{
    ...
    if (self) {
        ...
        self.conversation = [[EaseMob sharedInstance].chatManager conversationForChatter:chatter isGroup:NO];
        _as = [AppStatus sharedInstance];
    }
...
-(void)initHeader
{
    self.header = [[HeaderView alloc]initWithTitle:page_name_feedback navigationController:self.navigationController];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(aplicationNotification) name:UIApplicationDidBecomeActiveNotification object:nil];
...
-(void) viewDidAppear:(BOOL)animated{
    [EaseMobProcessor clearUnreadMessage:self.conversation];
}
...
- (UITableView *)tableView
{
    if (_tableView == nil) {
        ...
        _tableView.backgroundColor = [ColorUtils colorWithHexString:backgroud_color];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            if ([model isSender]) {
                if (_as.user.avatarUrl == nil || [_as.user.avatarUrl isEqualToString:@""]) {
                    model.headImageURL = [[NSBundle mainBundle] URLForResource:@"user_default_image_before_load" withExtension:@"png"];
                }else{
                    model.headImageURL = [[NSURL alloc] initWithString:_as.user.avatarUrl];
                }
                model.nickName = _as.user.name;
            }else{
                model.headImageName = @"shishangmao_icon";
            }
...
{
    EMMessage *locationMessage = [ChatSendHelper sendLocationLatitude:latitude longitude:longitude address:address toUsername:_conversation.chatter isChatGroup:NO requireEncryption:NO];
