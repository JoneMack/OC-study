...
+(void) searcherHdcsByOrganizationIds:(void(^)(NSArray *hdcs , NSError *error))completionBlock
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/hdcs/organizationIds,%@?hdcType=%d&mustRecommend=%@" ,[AppStatus sharedInstance].searcherUrl, organizationIdsStr , organizationFilter.selectedHdcTypeValue,@"false"]];
//    NSLog(@">>>>>>>url >>>%@" ,url);
