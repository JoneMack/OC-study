...
+(void)getOrganizationById:(void (^)(Organization *, NSError *))completionCallback organizationId:(int)organizationId
{
    HttpRequestFacade *requestFacade = [HttpRequestFacade sharedInstance];
    NSString *url = [NSString stringWithFormat:@"/organizations/ids,%d",organizationId];
    [requestFacade get:url completionBlock:^(NSString *json, NSError *err) {
        if (err == nil) {
...
+(void)getOrganizationsWithOrganizationFilter:(void (^)(Page *, NSError *))completionCallback
    ...
    HttpRequestFacade *requestFacade = [HttpRequestFacade sharedInstance];
    NSString *urlStr = [NSString stringWithFormat:@"/organizations?hdcType=%d&businessCirclesName=%@&currentPOI=%f,%f&orderType=%@&pageNo=%d" ,  organizationFilter.selectedHdcTypeValue ,
